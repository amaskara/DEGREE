...
elif ed_config.dataset == "maven":
    from template_generate_maven import eve_template_generator
    template_file = "template_generate_maven"
# fix random seed
...
# dev_set = GenDataset(ed_tokenizer, ed_config.max_length, ed_config.dev_finetune_file, ed_config.max_output_length)
...
ed_model.load_state_dict(torch.load(args.ed_model, map_location=f'cuda:{ed_config.gpu_device}'), strict = False)
...
for batch in DataLoader(test_set, batch_size=ed_config.eval_batch_size, shuffle=False, collate_fn=test_set.collate_fn):
    ...
    if args.gold_trigger:
        ...
        for event_type in vocab['event_type_itos']:
            ...
            final_outputs = [ed_tokenizer.decode(output, skip_special_tokens=False, clean_up_tokenization_spaces=True) for output in outputs]
